...
/// East Asian Wide and Fullwidth, which occupy two terminal cells.
const WIDE: &[(u32, u32)] = &[
    (0x1100, 0x115F),
    (0x231A, 0x231B),
    (0x2329, 0x232A),
...
/// Terminal cells one scalar occupies, ignoring any cluster it belongs to.
fn scalar_width(c: char) -> usize {
    match class(c) {
        // Combining marks, joiners and format characters take no space.
        Gcb::Extend | Gcb::Zwj | Gcb::Control | Gcb::Prepend => 0,
        _ => {
            if in_ranges(c as u32, WIDE) {
                2
            } else {
                1
            }
...
/// Terminal cells `src` occupies when printed.
///
/// This is a *different* question from [`count`], and the two disagree on
/// exactly the characters that made the distinction worth drawing: `🧑‍🧑‍🧒‍🧒` is one
/// character but occupies two cells. Column numbers use `count`, because a
/// column says which character; caret alignment uses this, because a caret is
/// laid out in cells.
///
/// The width of an emoji is not knowable from the text alone — it depends on
/// whether the terminal and font ligate the sequence. This assumes they do,
/// which is what current terminals generally manage. Where they do not, a
/// caret drifts left; nothing else is affected.
pub fn width(src: &str) -> usize {
    graphemes(src).map(cluster_width).sum()
}

fn cluster_width(cluster: &str) -> usize {
    let Some(first) = cluster.chars().next() else { return 0 };

    // A joined emoji sequence draws as one double-width glyph.
    if cluster.contains('\u{200D}') && is_ext_pict(first) {
        return 2;
    }
    // So does a regional-indicator pair, which draws as one flag.
    if class(first) == Gcb::RegionalIndicator {
        return 2;
    }
    // U+FE0F asks for emoji presentation, which is double width even when the
    // base character would otherwise be narrow — `❤️` against `❤`.
    if cluster.contains('\u{FE0F}') && is_ext_pict(first) {
        return 2;
    }
...
mod tests {
    ...
    #[test]
    fn width_and_count_are_different_questions() {
        // One character, two cells: exactly the case that motivates having both.
        assert_eq!(count("🧑‍🧑‍🧒‍🧒"), 1);
        assert_eq!(width("🧑‍🧑‍🧒‍🧒"), 2);
    }

    #[test]
    fn ascii_width_equals_its_count() {
        assert_eq!(width("var i32 ("), 9);
        assert_eq!(width(""), 0);
    }

    #[test]
    fn emoji_and_flags_take_two_cells() {
        assert_eq!(width("👍🏽"), 2);
        assert_eq!(width("🇹🇭"), 2);
        assert_eq!(width("❤️"), 2);
        assert_eq!(width("🎯"), 2);
    ...
    #[test]
    fn combining_marks_take_none_of_their_own() {
        assert_eq!(width("é"), 1);
        assert_eq!(width("a\u{0300}\u{0301}"), 1);
    }
    ...
    #[test]
    fn the_motivating_line_measures_correctly() {
        // `var i32 (🧑‍🧑‍🧒‍🧒) = '1' end var i32 ` — 30 characters, 31 cells.
        let lead = "var i32 (🧑‍🧑‍🧒‍🧒) = '1' end var i32 ";
        assert_eq!(count(lead), 30);
        assert_eq!(width(lead), 31);
    }
}
